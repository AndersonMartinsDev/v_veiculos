...
export class ProdutoCartaoComponent implements OnInit {
  @Input() veiculo!: VeiculoResumoModel;
  ...
  ngOnInit(): void {
    this.toNzUploadFile(this.veiculo.imagem);
  }

  detalhe(id: number) {
    this.detalhar.emit(id);
  }

  toNzUploadFile(image: Anexo) {
    if(image){
      ...
      this.fallback = `data:${fileIt.type};base64,${image.arquivo}`;
    }
  }
}
